...
[torch.nn.functional.scaled\_dot\_product\_attention](https://pytorch.org/docs/stable/generated/torch.nn.functional.scaled_dot_product_attention.html),
...
**Inputs**
  ...
  - **4**: `attention_mask` - two options available. `attention_mask` is ignored if `causal` is set to `True`. **Optional.**
    ...
    >   - at least 2 dimensional tensor of type *T* or `boolean` and shape numpy-broadcastable to `[N, ..., L, S]`. See \[Numpy Broadcast Rules\](../../broadcast-rules.md) for broadcast details.
    >   - a scalar of type *T* with value `0`. Scalar zero value signals that applying an attention mask is not necessary (similar to specifying attention\_mask=None in the provided pseudo-code).
...
**Dimensions**
  ...
  - `N, ...` - one or more batch dimensions. Each batch dimension should be either constant across the input tensors (query, key, and value), indicating that they have the same batch size, or they should be numpy-broadcastable to the same value. See \[Numpy Broadcast Rules\](../../broadcast-rules.md) for broadcast details.
