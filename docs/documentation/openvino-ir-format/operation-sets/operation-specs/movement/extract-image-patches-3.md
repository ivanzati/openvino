---
sidebar_label: 'ExtractImagePatches'
format: md
---
...
The "auto\_pad" attribute has no effect on the size of each patch, it determines how many patches are extracted.
...
**Attributes**
  ...
  - *auto\_pad*
      - **Description**: *auto\_pad* how the padding is calculated. Possible values:
          - *same\_upper (same\_lower)* the input is padded by zeros to match the output size. In case of odd padding value an extra padding is added at the end (at the beginning).
