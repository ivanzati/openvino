...
**Attributes**:
  ...
  - *mode*
      ...
      - **Range of values**:
          - *numpy* - numpy broadcasting rules, aligned with ONNX Broadcasting. Description is available in [ONNX docs](https://github.com/onnx/onnx/blob/master/docs/Broadcasting.md) .; only one-directional broadcasting is applied from `data` to `target_shape`. If this attribute value is used, then the 3rd input for the operation shouldn't be provided.
...
**Inputs**:
  ...
  - **2**: `target_shape` - 1D tensor of type *T\_SHAPE* describing output shape. **Required.**
  - **3**: `axes_mapping` - 1D tensor of type *T\_SHAPE* describing a list of axis indices, each index maps an axis from the 1st input tensor `data` to axis in the output. The index values in this tensor should be sorted, that disables on-the-fly transpositions of input `data` tensor while the broadcasting. `axes_mapping` input is needed for `mode` equal to *explicit* only.
