...
*DepthToSpace* operation permutes elements from the input tensor with shape `[N, C, D1, D2, ..., DK]`, to the output tensor where values from the input depth dimension
...
**Attributes**
  ...
  - *mode*
      ...
      - **Range of values**:
          - *blocks\_first*: the input depth is divided to `[block_size, ..., block_size,  new_depth]`
          - *depth\_first*: the input depth is divided to `[new_depth, block_size, ..., block_size]`
      - **Type**: `string`
      - **Required**: *yes*
