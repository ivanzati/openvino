...
output\[i\_0, ..., [i]()\{K-2\},:,...,:\] = data\[indices\[i\_0, ..., [i]()\{K-2\}\],:,...,:\]
...
**Attributes**:
  ...
  - *batch\_dims*
      - **Description**: *batch\_dims* (denoted as `b`) is a leading number of dimensions of `data` tensor
        ...
        In case of non-default value for *batch\_dims*, the output shape is calculated as
        `(multiplication of indices.shape[:b]) + indices.shape[b:-1] + data.shape[(indices.shape[-1] + b):]`.
        
        Note
        
        The calculation of output shape is incorrect for non-default *batch\_dims* value greater than one.
        For correct calculations use the \[GatherND\_8\](gather-8.md) operation.
...
Example 4 shows how *GatherND* operates gathering elements for non-default *batch\_dims* value:
...
Example 5 shows how *GatherND* operates gathering slices for non-default *batch\_dims* value:
...
for non-default *batch\_dims* value:
...
**Inputs**:
  ...
  - **2**: `indices` tensor of type *T\_IND*. This is a tensor of a rank not lower than 1.
...
**Types**
  ...
  - *T\_IND*: any supported integer types.
