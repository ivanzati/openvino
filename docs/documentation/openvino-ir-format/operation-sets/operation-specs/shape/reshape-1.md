...
**Attributes**:
  ...
  - *special\_zero*
      - **Description**: *special\_zero* controls how zero values in `shape` are interpreted. If *special\_zero* is `false`, then `0` is interpreted as-is which means that output shape will contain a zero dimension at the specified location. Input and output tensors are empty in this case. If *special\_zero* is `true`, then all zeros in `shape` implies the copying of corresponding dimensions from `data.shape` into the output shape *(left aligned)*.
...
**Inputs**:
  ...
  - **2**: `shape` 1D tensor of type *T\_SHAPE* describing output shape. **Required.**
...
**Types**
  ...
  - *T*: any numeric type.
  - *T\_SHAPE*: any supported integer type.
