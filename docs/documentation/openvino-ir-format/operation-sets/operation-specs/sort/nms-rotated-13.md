...
5.  Calculate `union_area` (the common area of `B1` and `B2`), union\_area = B1\_area + B2\_area.
...
**Attributes**:
  ...
  - *sort\_result\_descending*
      - **Description**: *sort\_result\_descending* is a flag that specifies whether it is necessary to sort selected boxes across batches or not.
...
**Inputs**:
  ...
  - **3**: `max_output_boxes_per_class` - scalar or 1D tensor with 1 element of type *T\_MAX\_BOXES* specifying the maximum number of boxes to be selected per class. **Required.**
  - **4**: `iou_threshold` - scalar or 1D tensor with 1 element of type *T\_THRESHOLDS* specifying intersection over union threshold. **Required.**
  - **5**: `score_threshold` - scalar or 1D tensor with 1 element of type *T\_THRESHOLDS* specifying minimum score to consider box for the processing. **Required.**
