...
**TensorIterator attributes**:
  ...
  - **Port map**:
    ...
    *port\_map* is a set of rules to map input or output data tensors of `TensorIterator` layer onto `body` data tensors. The `port_map` entries can be `input` and `output`. Each entry describes a corresponding mapping rule.
    
      - **Port map attributes**:
          - *external\_port\_id*
              - **Description**: *external\_port\_id* is a port ID of the `TensorIterator` layer.
              - **Range of values**: indexes of the *TensorIterator* outputs
              - **Type**: `int`
              - **Default value**: None
              - **Required**: *yes*
          - *internal\_layer\_id*
              - **Description**: *internal\_layer\_id* is a *Parameter* or *Result* layer ID inside the `body` network to map to.
  ...
  - **Back edges**:
    ...
    *back\_edges* is a set of rules to transfer tensor values from `body` outputs at one iteration to `body` parameters at the next iteration. Back edge connects some *Result* layer in `body` to *Parameter* layer in the same `body`.
