...
**Attributes**
  ...
  - *eps*
      - **Description**: *eps* is the number applied by *eps\_mode* function to the sum of squares to avoid division by zero when normalizing the value.
...
**Inputs**
  ...
  - **2**: `axes` - Axis indices of `data` input tensor, along which L2 reduction is calculated. A scalar or 1D tensor of unique elements and type *T\_IND*. The range of elements is `[-r, r-1]`, where `r` is the rank of `data` input tensor. **Required.**
