...
Similar to *NV12toRGB* but output channels for each pixel are reversed so that the first channel is `blue`, the second one is `green`, the last one is `red`. See detailed conversion formulas in the \[NV12toRGB description\](nv12-to-rgb-8.md).

**Inputs:**

Same as specified for \[NV12toRGB\](nv12-to-rgb-8.md) operation.
