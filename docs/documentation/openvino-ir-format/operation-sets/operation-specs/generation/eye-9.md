...
**Inputs**:
  ...
  - **1**: `num_rows` - scalar or 1D tensor with 1 non-negative element of type *T\_NUM* describing the number of rows in matrix. **Required.**
  - **2**: `num_columns` - scalar or 1D tensor with 1 non-negative element of type *T\_NUM* describing the number of columns in matrix. **Required.**
  - **3**: `diagonal_index` - scalar or 1D tensor with element of type *T\_NUM* describing the index of the diagonal to be populated. A positive value refers to an upper diagonal and a negative value refers to a lower diagonal. Value `0` populates the main diagonal. If `diagonal_index` is a positive value and is not smaller than `num_rows` or if `diagonal_index` is a negative value and is not larger than `num_columns`, the matrix will be filled with only zeros. **Required.**
  - **4**: `batch_shape` - 1D tensor with non-negative values of type *T\_NUM* defines leading batch dimensions of output shape. If `batch_shape` is an empty list, *Eye* operation generates a 2D tensor (matrix). This input is optional, and its default value equal to an empty tensor.

**Outputs**:

  - **1**: A tensor with the type specified by the *output\_type* attribute. The shape is `batch_shape + [num_rows, num_columns]`

**Types**

  - *T\_NUM*: `int32` or `int64`.
