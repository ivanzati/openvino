...
1.  Multiply box coordinates with *spatial\_scale* to produce box coordinates relative to the input feature map size.
...
**Attributes**
  ...
  - *pooled\_w*
      ...
      - **Required**: *yes*
  - *sampling\_ratio*
      - **Description**: *sampling\_ratio* is the number of bins over height and width to use to calculate each output feature map element. If the value is equal to 0 then use adaptive number of elements over height and width: `cei (roi_height / pooled_h)` and `ceil(roi_width / pooled_w)` respectively.
