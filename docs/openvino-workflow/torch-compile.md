---
sidebar_label: 'PyTorch Deployment via "torch.compile"'
format: md
---
...
[torch\_dynamo\_backends entrypoint](https://pytorch.org/docs/stable/torch.compiler_custom_backends.html#registering-custom-backends).
...
![torch.compile execution diagram](img/torch_compile_backend_openvino.svg)
...
enable model caching, set the cache directory etc. You can use a dictionary of the available options:
  ...
  - `aot_autograd` - enables aot\_autograd graph capture. The aot\_autograd graph capture
    is needed to enable dynamic shapes or to finetune a model. For models with dynamic
    shapes, it is recommended to set this option to `True`. By default, aot\_autograd
    is set to `False`.
...
1.  Install the PyTorch nightly wheel file - [2.1.0.dev20230713](https://download.pytorch.org/whl/cpu/torch-2.1.0%2Bcpu-cp38-cp38-win_amd64.whl) ,
...
you can refer to [TorchServe github repository.](https://github.com/pytorch/serve). With OpenVINO `torch.compile` integration into TorchServe you can serve
...
available in [TorchServe examples.](https://github.com/pytorch/serve/tree/master/examples/pt2/torch_compile_openvino) and in a [use case app](https://github.com/pytorch/serve/tree/master/examples/usecases/llm_diffusion_serving_app).
...
[Stable Diffusion WebUI repository.](https://github.com/openvinotoolkit/stable-diffusion-webui/wiki/Installation-on-Intel-Silicon)
...
1.  Neural Network Compression Framework ([NNCF](https://docs.openvino.ai/2026/openvino-workflow/model-optimization.html)). NNCF offers advanced algorithms for post-training quantization and weights compression in the OpenVINO toolkit.
...
The Neural Network Compression Framework ([NNCF](https://docs.openvino.ai/2026/openvino-workflow/model-optimization.html)) implements advanced quantization and weights compression algorithms, which can be applied to `torch.fx.GraphModule` to speed up inference
...
For further details, please see the [documentation](https://docs.openvino.ai/2026/openvino-workflow/model-optimization.html)
and a [tutorial](https://github.com/openvinotoolkit/nncf/tree/develop/examples/post_training_quantization/torch_fx/resnet18).
...
[PyTorch 2 Export Post Training Quantization with X86 Backend through Inductor](https://pytorch.org/tutorials/prototype/pt2e_quant_ptq_x86_inductor.html)
...
The `torch.compile` feature is part of PyTorch 2.0, and is based on:
  ...
  - **TorchDynamo** - a Python-level JIT that hooks into the frame evaluation API in CPython,
    ...
    [Guards](https://pytorch.org/docs/2.0/dynamo/guards-overview.html) to ensure the
