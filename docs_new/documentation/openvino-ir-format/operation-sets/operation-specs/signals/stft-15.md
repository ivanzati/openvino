---
sidebar_label: 'Short Time Fourier Transformation for real-valued input (STFT)'
format: md
---
...
**Attributes**:
  ...
  - *transpose\_frames*
      - **Description**: Flag to set output shape layout. If true the `frames` dimension is at out\_shape\[-2\], otherwise it is at out\_shape\[-3\].
      - **Range of values**:
...
**Inputs**
  ...
  - **1**: `signal` - Tensor of type *T* and 1D shape \[signal\_size\] or 2D shape \[batch, signal\_size\] with signal data for the STFT. **Required.**
  - **2**: `window` - Tensor of type *T* and 1D shape \[window\_length\], specifying the window values for the signal slice multiplication. **Required.**
...
*Example 1D signal, transpose\_frames=false:*
...
*Example 1D signal, transpose\_frames=true:*
...
*Example 2D signal, transpose\_frames=false:*
...
*Example 2D signal, transpose\_frames=true:*
