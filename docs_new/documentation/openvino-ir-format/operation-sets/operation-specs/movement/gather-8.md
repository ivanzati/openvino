...
**Attributes**:
  ...
  - *batch\_dims*
      - **Description**: *batch\_dims* (also denoted as `b`) is a leading number of dimensions of `data` tensor
...
Example 1 with default *batch\_dims* value:
...
Example 2 with non-default *batch\_dims* value:
...
Example 3 with non-default *batch\_dims* value:
...
Example 4 with *axis* > *batch\_dims*:
...
Example 5 with negative *batch\_dims* value:
...
**Inputs**
  ...
  - **2**: `indices` tensor of type *T\_IND* with indices to gather. 0D tensor (scalar) for indices is also allowed.
  ...
  - **3**: Scalar or 1D tensor `axis` of *T\_AXIS* type is a dimension index to gather data from. For example,
...
**Types**
  ...
  - *T\_AXIS*: any supported integer types.
