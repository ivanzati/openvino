...
**Inputs**
  ...
  - **2**: `start` - 1D tensor of type *T\_IND*. Indices corresponding to axes in `data`.
  ...
  - **3**: `stop` - 1D, type *T\_IND*, similar to `start`.
  ...
  - **4**: `step` - 1D tensor of type *T\_IND* and the same shape as `start` and `stop`.
  ...
  - **5**: `axes` - 1D tensor of type *T\_AXIS*.
...
**Types**
  ...
  - *T\_AXIS*: any supported integer type.
