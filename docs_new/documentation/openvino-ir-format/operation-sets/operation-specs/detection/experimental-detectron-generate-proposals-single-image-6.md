---
sidebar_label: 'ExperimentalDetectronGenerateProposalsSingleImage'
format: md
---
...
3.  Removes predicted boxes with either height or width \< *min\_size*.
4.  Sorts all `(proposal, score)` pairs by score from highest to lowest; order of pairs with equal scores is undefined.
5.  Takes top *pre\_nms\_count* proposals, if total number of proposals is less than *pre\_nms\_count* takes all proposals.
6.  Applies non-maximum suppression with *nms\_threshold*.
7.  Takes top *post\_nms\_count* proposals and returns these top proposals and their scores. If total number of proposals is less than *post\_nms\_count* returns output tensors filled with zeroes.
