---
sidebar_label: 'ReduceLogicalAnd'
format: md
---
...
**Inputs**
  ...
  - **1**: `data` - A tensor of type *T\_BOOL* and arbitrary shape. **Required.**
  - **2**: `axes` - Axis indices of `data` input tensor, along which the reduction is performed. A scalar or 1D tensor of unique elements and type *T\_IND*. The range of elements is `[-r, r-1]`, where `r` is the rank of `data` input tensor. **Required.**

**Outputs**

  - **1**: The result of *ReduceLogicalAnd* function applied to `data` input tensor. A tensor of type *T\_BOOL* and `shape[i] = shapeOf(data)[i]` for all `i` dimensions not in `axes` input tensor. For dimensions in `axes`, `shape[i] == 1` if `keep_dims == true`; otherwise, the `i`-th dimension is removed from the output.
