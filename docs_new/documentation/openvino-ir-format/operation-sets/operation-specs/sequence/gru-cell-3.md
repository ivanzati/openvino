...
**Attributes**
  ...
  - *activations\_alpha, activations\_beta*
      - **Description**: *activations\_alpha, activations\_beta* functions attributes
  ...
  - *linear\_before\_reset*
      - **Description**: *linear\_before\_reset* flag denotes if the layer behaves according to the modification
        of *GRUCell* described in the formula in the [ONNX documentation](https://github.com/onnx/onnx/blob/main/docs/Operators.md#GRU).
...
**Inputs**
  ...
  - **5**: `B` - 1D tensor of type *T*. If *linear\_before\_reset* is set to 1, then the shape is `[4 * hidden_size]` -the sum of biases for z and r gates (weights and recurrence weights), the biases for h gate are placed separately.
