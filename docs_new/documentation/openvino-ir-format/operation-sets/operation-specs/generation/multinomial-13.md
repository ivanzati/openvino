...
Given a list of probabilities x1, x2, ..., xn:
  ...
  - If *log\_probs* is true:
      - For each probability x, replace it with a value \(e^\{x\}\).
  - Create an array - discrete CDF ([Cumulative Distribution Function](https://hal.science/hal-00753950/file/PEER_stage2_10.1016%252Fj.spl.2011.03.014.pdf)) - the cumulative sum of those probabilities, ie. create an array of values where the ith value is the sum of the probabilities x1, ..., xi.
  ...
  - If *with\_replacement* is False (sampling without replacement):
  ...
  - Convert the output indices to *convert\_type*.
...
**Attributes**:
  ...
  - `convert_type`
      - **Description**: the type of the output. Determines generation algorithm and affects resulting values. Output numbers generated for different values of *convert\_type* may not be equal.
  ...
  - `with_replacement`
      ...
      - **Range of values**: true, false
          ...
          - `false` - class indices will not repeat in the output and the size of `probs`' `class_size` dimension is required to be larger or equal to *num\_samples* value. Might affect performance.
      - **Type**: bool
      - **Required**: *Yes*
  - `log_probs`
...
**Inputs**:
  ...
  - **1**: `probs` - A 2D tensor of type T\_IN and shape \[batch\_size, class\_size\] with probabilities. Allowed values depend on the *log\_probs* attribute. The values are internally normalized to have values in the range of \[0, 1\] with the sum of all probabilities in the given batch equal to 1. **Required.**
  - **2**: `num_samples` - A scalar or 1D tensor with a single element of type T\_SAMPLES specifying the number of samples to draw from the multinomial distribution. **Required.**

**Outputs**:

  - **1**: `output`- A tensor with type specified by the attribute *convert\_type* and shape `[batch_size, num_samples]`.

**Types**

  - **T\_IN**: any supported floating-point type.
  - **T\_SAMPLES**: 32-bit or 64-bit integers.
