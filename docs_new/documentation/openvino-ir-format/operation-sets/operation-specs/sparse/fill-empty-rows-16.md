---
sidebar_label: 'SparseFillEmptyRows'
format: md
---
...
The output sparse tensor will be in row-major order and will have the same dense shape as the dense\_shape input, but with updated output\_indices and output\_values.
...
**Inputs**:
  ...
  - **2**: `dense_shape` 1D tensor of type *T\_IDX* indicating the shape of the 2D dense tensor. **Required.**

  -   - **3**: `indices` 2D tensor of type *T\_IDX* and non-negative values indicating the positions at which `values` are placed in the sparse tensor. **Required.**  
...
**Outputs**:
  ...
  -   - **1**: `output_indices` 2D tensor of type *T\_IDX* indicating the positions at which `output_values` are placed in the sparse tensor.  
        It is of shape `[M', 2]`, where:
...
The output sparse tensor will be in row-major order and will have the same dense shape as the dense\_shape input.
