...
**Outputs**:
  ...
  - **2**: Output tensor with top *k* indices for each slice along *axis* dimension of type *T\_IND*. The shape of the tensor is the same as for the 1st output, that is `[input1.shape[0], ..., input1.shape[axis-1], k, input1.shape[axis+1], ...]`.
