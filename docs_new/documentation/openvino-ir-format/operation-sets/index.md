---
sidebar_label: 'Operation Sets in OpenVINO'
format: md
---
...
![image](img/small_IR_graph_demonstration.png)
...
For more information, refer to the complete description of the supported operation sets in the \[Available Operation Sets\](operation-sets/available-opsets.md) article.

## How to Read Opset Specification

In the \[Available Operation Sets\](operation-sets/available-opsets.md) there are opsets and there are operations.
...
For example, the ReLU entry point in \[opset1\](operation-sets/available-opsets/opset1.md) refers to \[ReLU-1\](operation-sets/operation-specs/activation/relu-1.md) as the versioned name.
Meanwhile, ReLU in opset2 refers to the same ReLU-1 and both ReLU operations are the same operation and it has a single \[description\](operation-sets/operation-specs/activation/relu-1.md), which means that `opset1` and `opset2` share the same operation `ReLU`.
