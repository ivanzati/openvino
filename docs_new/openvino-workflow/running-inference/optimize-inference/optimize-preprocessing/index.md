---
sidebar_label: 'Optimize Preprocessing'
format: md
---
...
![image](img/preprocess_not_fit.png)
...
1.  **Tensor** - declares user data format, like shape, \[layout\](optimize-preprocessing/layout-api-overview.md), precision, color format from actual user's data.
2.  **Steps** - describes sequence of preprocessing steps which need to be applied to user data.
3.  **Model** - specifies model data format. Usually, precision and shape are already known for model, only additional information, like \[layout\](optimize-preprocessing/layout-api-overview.md) can be specified.
...
Below is all the specified input information:
  ...
  - \[Layout\](optimize-preprocessing/layout-api-overview.md) is "NHWC". It means: `height=480`, `width=640`, `channels=3`.
  - Color format is `BGR`.

### Declaring Model Layout

Model input already has information about precision and shape. Preprocessing API is not intended to modify this. The only thing that may be specified is input data \[layout\](optimize-preprocessing/layout-api-overview.md)
...
## Additional Resources
  ...
  - \[Preprocessing Details\](optimize-preprocessing/preprocessing-api-details.md)
  - \[Layout API overview\](optimize-preprocessing/layout-api-overview.md)
