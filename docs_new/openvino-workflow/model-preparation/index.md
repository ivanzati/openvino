---
sidebar_label: 'Conventional Model Preparation'
format: md
---
...
[Kaggle](https://www.kaggle.com/models), [Hugging Face](https://huggingface.co/), and
[Torchvision models](https://pytorch.org/hub/). Now you have two options:

  - Skip model conversion and \[run inference\](running-inference.md)
    ...
    For PyTorch models, you can use the dedicated \[torch.compile\](./torch-compile.md)
    implementation.

  - Explicitly \[convert the model to OpenVINO IR\](model-preparation/convert-model-to-ir.md).
    ...
    cases. Explicit conversion can be done in two ways:
      ...
      - the [Python API functions](#convert-a-model-with-python-convert-model)
        (`openvino.convert_model` and `openvino.save_model`)
      - the [ovc](#convert-a-model-in-cli-ovc) command line tool.
    
    Once saved as \[OpenVINO IR\](model-preparation/convert-model-to-ir.md)
    (a set of `.xml` and `.bin` files), the model may be deployed with maximum performance.
    Because it is already optimized for
    \[OpenVINO inference\](running-inference.md),
    it can be read, compiled, and inferred with no additional delay.

Note

For PyTorch and JAX/Flax models, [Python API](#convert-a-model-with-python-convert-model) is the only
conversion option.
...
For more information on each function, see the \[OpenVINO workflow\](../openvino-workflow.md)
page.
...
or further \[optimized with NNCF\](model-optimization.md) before saving.
...
\[Post-training Optimization\](model-optimization-guide/quantizing-models-post-training.md) to achieve more efficient inference and
a smaller model.
...
formats to
\[OpenVINO IR\](model-preparation/convert-model-to-ir.md),
...
the source model type:
  ...
  - \[Convert different model formats to the ov.Model format\](model-preparation/convert-model-to-ir.md).
  - \[Review all available conversion parameters\](model-preparation/conversion-parameters.md).

To achieve the best model inference performance and more compact OpenVINO IR representation
follow:

  - \[Post-training optimization\](model-optimization-guide/quantizing-models-post-training.md)
  - \[Model inference in OpenVINO Runtime\](running-inference.md)
